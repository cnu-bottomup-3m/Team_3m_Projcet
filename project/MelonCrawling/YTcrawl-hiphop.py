...
def proxx():
    ...
    for i in range(100):
        ...
        try:
            ...
            url = 'https://www.youtube.com/results?search_query=' + hiphop_json["entries"][i]["artist"] + '+' + \
                  hiphop_json["entries"][i]["title"] + '+' + 'audio'
            ...
            continue
    a = 0
    for d in hiphop_json['entries']:
        d['videoId'] = str(title[a])
        a += 1

    with open('/var/www/html/json/melon_day_hiphop.json', 'w', encoding="utf-8") as make_file:
        json.dump(hiphop_json, make_file, ensure_ascii=False, indent="\t")
