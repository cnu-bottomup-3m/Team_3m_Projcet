...
def proxx():
    ...
    for i in range(100):
        ...
        try:
            ...
            url = 'https://www.youtube.com/results?search_query=' + afolk_json["entries"][i]["artist"] + '+' + \
                  afolk_json["entries"][i]["title"] + '+' + 'audio'
            # print(session.get(ipconfig).text)
            time.sleep(2)
            ...
            continue
    a = 0
    for d in afolk_json['entries']:
        d['videoId'] = str(title[a])
        a += 1

    with open('/var/www/html/json/melon_day_ab_folk.json', 'w', encoding="utf-8") as make_file:
        json.dump(afolk_json, make_file, ensure_ascii=False, indent="\t")
