...
def proxx():
    ...
    for i in range(100):
        ...
        try:
            ...
            url = 'https://www.youtube.com/results?search_query=' + top100_json["entries"][i]["artist"] + '+' + \
                  top100_json["entries"][i]["title"] + '+' + 'audio'
            # print(session.get(ipconfig).text)
            time.sleep(2)
    ...
    a = 0
    for d in top100_json['entries']:
        d['videoId'] = str(title[a])
        a += 1

    with open('/var/www/html/json/melon_top_100.json', 'w', encoding="utf-8") as make_file:
