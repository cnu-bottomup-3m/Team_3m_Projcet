...
function show_melon_apop_chart(this_id) {
    ...
    if (multi_selcet) { //멀티셀렉트가 켜져있으면
        alert("다중 선택이 켜져있는 상태에서 다른차트로 이동할수없습니다.")
        return;
    }
    ...
    $.getJSON("/json/melon_day_ab_pop.json", function (json) {
        for (var i = 0; i < json.entries.length; i++) {
            $(b).append('<tr class="chart" id=' + json.entries[i].videoId + '>' + '<td class="rank">' + json.entries[i].rank + '</td>' +
                '<td class="thumbnail">' + '<img class="albumcover" width="70" height="70" src="' + json.entries[i].imgsrc +
...
$('#chartlist-ul').on('click', function (event) {
...
function add_playlist(event) { //플레이리스트추가 부분   8월3일 수정
    ...
    var title = target.children[3].innerText;
...
function showtube(ev) {
    ...
    var title = parent.children[3].innerText;
    var artist = parent.children[2].innerText
    var name = String(artist + " - " + title);
...
$('#multi-select').on('click', function () { //다중선택을 눌럿을떄
    ...
    if (multiOn) {
        multi_selcet = true;
        $('.chartlist-ul .onplay').css("display", "none");
...
$('#chartlist-ul').on('click', function (ev) {
    if (!multi_selcet) //true가아니면 
        return; //종료

    var tmp = ev.target.parentElement.children[5];
...
var totalOn = false;
$('#total-select').on('click', function () { //전체선택
    var text = '<img class="selected-music" src="/img/finger.png">'
    var total_select = document.querySelector('#total-select');
    var element = $('#chartlist-ul tr .addtolist'); //추가할 element를 가져옴
...
function add_selected_playlist(node) { //플레이리스트추가 부분   8월3일 수정
    ...
    var parent = node;
    var artist = node.children[2].innerText;
    var title = node.children[3].innerText;
    var videoId = node.id;
    var name = String(artist + " - " + title);
