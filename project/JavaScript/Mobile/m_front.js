...
function show_melon_apop_chart(this_id) {
    ...
    if(multi_selcet){ //멀티셀렉트가 켜져있으면
        alert("여러개 선택이 켜져있는 상태에서 다른차트로 이동할수없습니다.")
        return;
    }
...
//    var tmp=ev.target.parentElement.childNodes[4];
   ...
   var title=p_node.childNodes[2].firstElementChild.innerText;
   var artist=p_node.children[2].lastElementChild.innerText;
...
function add_selected_playlist(node) { //플레이리스트추가 부분   8월3일 수정
    ...
    var parent=node;
    var title=parent.childNodes[2].firstElementChild.innerText;
...
function savethislist() {
    ...
    forsending.title = txtforlist.value;
    ...
    $.ajax({
        ...
        success: function (result) {
            var temtex = '<li><span class="mypl">' + txtforlist.value + '</span><button class="mypldltbtn" onclick="playlist_delete(event)">X</button></li>';
            $("#listofmyplaylist").append(temtex);
            alert(result);
        },
...
function playlist_delete(ev) {
    event = ev;
    $("#listofmyplaylist").removeChild(event.target.parentNode);
}
